...
class JPADelegatingClassFileTransformer implements ClassTransformer {
    private final Map<String, List<PersistenceUnitMetadata>> toTransform;
    private final Set<String> processed;

    public JPADelegatingClassFileTransformer(final Map<String, List<PersistenceUnitMetadata>> toTransform) {
        this.toTransform = Map.copyOf(toTransform);
        ...
        PrivilegedAction<ByteBuffer> privilegedAction =
                new PrivilegedAction<ByteBuffer>() {
                    ...
                    public ByteBuffer run() {
                        if (toTransform.containsKey(className)) {
                            ...
                            if (processed.add(className)) {
                                ...
                                for (PersistenceUnitMetadata persistenceUnitMetadata : toTransform.get(className)) {
