...
@RunAsClient
public class Log4jConfigLogRouterDisabledTestCase extends AbstractLogRouterTest {
    private static final String DEPLOYMENT_1 = "test-log4j-config--disabled-logging-1.war";
    private static final String DEPLOYMENT_2 = "test-log4j-config-disabled-logging-2.war";
    private static final String DEP1_FILE_NAME = "log4j-test1-disabled-json.log";
    private static final String DEP2_FILE_NAME = "log4j-test2-disabled-json.log";

    public Log4jConfigLogRouterDisabledTestCase() {
    ...
    public void testDeployment1(@ArquillianResource URL url) throws Exception {
        final String suffix = "log4j-test-dep-1";
    ...
    public void testDeployment2(@ArquillianResource URL url) throws Exception {
        final String suffix = "log4j-test-dep-2";
        final UrlBuilder builder = UrlBuilder.of(url, "log");
        builder.addParameter("suffix", suffix);
        performCall(builder.build());
        ...
        Assert.assertEquals(4L, dep1Logs.stream().filter(jsonObject ->
        ...
        Assert.assertEquals(8L, dep2Logs.stream().filter(jsonObject -> jsonObject.getString("message").contains(suffix)).count());
